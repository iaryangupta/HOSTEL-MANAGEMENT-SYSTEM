-- creating hostel database
create database HOSTEL;
use HOSTEL;

-- table to store details of hostel wardens
create table WARDEN (
...
Room_Type char(1), 
Floor_No int, W_id char(3), 
foreign key (W_id) references WARDEN(W_id));

-- table to store details of students living in hostel
...
-- table to store entry/exit details of student
...
-- table to record hostel fees payment
create table H_PAYMENT (
Start_Date date, 
Roll_No int, 
End_Date date, 
Amount int, 
primary key (Start_date, Roll_No), 
foreign key(Roll_No) references STUDENT(Roll_No));

-- table to record mess fees payment
...
-- table to store any grievances registered by students
